...
IMPLEMENTATION = M1_FOUNDATION_HARDENING_COMPLETE_WITHIN_APPROVED_THREAT_MODEL; M1_OPPORTUNITY_EVIDENCE_SUBSTRATE_IMPLEMENTATION_COMPLETE; M2_BATCH_2_IMPLEMENTATION_COMPLETE_REPAIRED_PENDING_FINAL_ACCEPTANCE
...
ACCEPTANCE_REVIEW = M1_FOUNDATION_PASS; M1_OPPORTUNITY_EVIDENCE_SUBSTRATE_PASS; M2_BATCH_1_PASS; M2_BATCH_2_FINAL_RE_REVIEW_PENDING

M1_STATUS = FOUNDATION_ACCEPTED_AND_ARCHIVED; OPPORTUNITY_EVIDENCE_SUBSTRATE_ACCEPTED_AND_ARCHIVED

M2_STATUS = IN_PROGRESS

NEXT = M2_FINAL_READ_ONLY_ACCEPTANCE_RE_REVIEW
...
已完成：產品及實施規劃、獨立 Git baseline、T02 架構契約、T01 M1 能力選型，以及依核准 threat model 完成的 M1 Foundation hardening。Static symlink/hard-link、Darwin recovery-path、stale-recovery concurrency 與 non-Darwin stale-claim crash/takeover tests 通過；same-user path-swap 仍未防護，明列為 M1 non-goal/future hardening，並保留為 skipped fixture，不宣稱已修復。Foundation suite 為 17 PASS / 1 SKIPPED / 0 FAIL。H5 fresh Electron startup acceptance 已有 automated app-layer evidence 與使用者實際觀察到的 visible window：fresh PID 79150 載入 Career 2.0 main entry、renderer、preload/IPC，synthetic private root 的 store READY version 1，完成 8 秒可見觀察並以 code 0 / signal null 結束。Read-only M1 acceptance review 已 PASS；`establish-m1-foundation` 已 sync 並 archive 至 `openspec/changes/archive/2026-09-16-establish-m1-foundation`。命名的 `m1-opportunity-evidence-substrate` 已完成最小 Opportunity/JD 與 Career Evidence/revision implementation、validation、read-only acceptance 與 archive，位於 `openspec/changes/archive/2026-09-18-m1-opportunity-evidence-substrate`；M2 planning 已通過 read-only acceptance，baseline close-out 完成，M2 Batch 1 scoped implementation 已完成 10/26 tasks 並通過獨立 read-only acceptance，Batch 1 已 ACCEPTED；M2 Batch 2 implementation 已完成其餘 8 個 implementation/validation tasks 並修復 final acceptance findings A/B，累計 25/26，包含 positioning lifecycle、complete-store backup coverage、purpose-specific IPC、minimal Opportunity surface、renderer begin/cancel controls、current-result preservation、fixtures/regressions 與 bounded synthetic vertical-slice probe；renderer repair 後 bundled full suite 為 74 PASS / 1 SKIPPED / 0 FAIL，final independent read-only Batch 2 acceptance re-review 尚未執行。詳細狀態見 [PROGRESS.md](PROGRESS.md)。完整 M1 Workspace/product-domain features 與部署仍未開始；M2 full acceptance 與 later batches 仍未完成。

目前交付順序：Plan Freeze → Career 2.0 independent Git baseline → T02 Phase 0 architecture → T01 M1 capability selection → M1 Foundation implementation/hardening → real Electron startup acceptance → read-only M1 review → OpenSpec sync/archive → commit/push → named `m1-opportunity-evidence-substrate` implementation → read-only prerequisite acceptance → prerequisite OpenSpec archive/commit/push → M2 planning acceptance/baseline close-out → M2 Intelligence Batch 1 implementation → Batch 1 independent acceptance → M2 Intelligence Batch 2 implementation → renderer acceptance repair → final independent read-only acceptance → M1 Workspace/product vertical slices → later M2 batches。Batch 2 renderer repair is complete and remains pending final acceptance re-review；下一個 M2 batch 尚未開始。
