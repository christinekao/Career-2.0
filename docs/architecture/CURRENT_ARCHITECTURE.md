...
| 本輪狀態 | Foundation hardening 已依核准 threat model 完成並通過 read-only M1 acceptance review；static path confinement、Darwin recovery-path、stale-recovery concurrency 與 non-Darwin stale-claim crash/takeover regressions 通過。Active same-user path swap 是未防護的 future-hardening non-goal。H5 fresh Electron GUI acceptance 已完成並具 automated + human-observed evidence；OpenSpec M1 changes 已 sync/archive。`m1-opportunity-evidence-substrate` 的最小 runtime、validation 與獨立 acceptance 已完成並 archive；`m2-intelligence-vertical-slice` planning 已 accepted，Batch 1 已 ACCEPTED，Batch 2 implementation 已完成 25/26 tasks，獨立 read-only acceptance 尚未執行；M2 `IN_PROGRESS`，later batches NOT_STARTED。完整 product-domain runtime schema/workflow、full M2 acceptance 與 later M2 slices 尚未被宣稱完成 |
...
狀態為 `PLANNING_ACCEPTED / BATCH_1_ACCEPTED / BATCH_2_IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE / IN_PROGRESS`。它只規劃單一
...
M2 的硬性前置是 [`m1-opportunity-evidence-substrate`](../../openspec/changes/archive/2026-09-18-m1-opportunity-evidence-substrate/)，其最小 implementation、validation 與獨立 acceptance 已完成並 archive；該 gate 已滿足。M2 詳細的 identity、taxonomy、traceability、execution、migration、backup authority 與 minimal surface contract 由 active change 的 proposal/spec/design/tasks 擁有；M2 planning review 已 PASS，baseline close-out 已完成，Batch 1 已完成 10/26 scoped tasks 並通過獨立 acceptance，Batch 2 implementation 已完成其餘 8 個 tasks，累計 25/26，獨立 read-only acceptance 尚未執行；later batches NOT_STARTED。本文件只保留 current boundary 與 dependency，不複製整份 M2 spec。

### M2 Batch 2 implementation

The current service boundary is provider-independent and remains inside the
existing main-process writer. `src/main/execution.cjs` owns the frozen
request/response contract, finite lifecycle/error vocabularies, timeout,
cancellation, retry, idempotency, duplicate handling, stale rejection, and
surface-state projection. `src/main/persistence.cjs` owns the additive
`intelligence_executions` table and atomic state/result transitions; candidate
payloads are validated through the accepted Batch 1 intelligence validators
before persistence, and execution completion creates only candidate
...
executor receives no database handle or private-root path. Independent
read-only Batch 2 acceptance remains pending.
...
- Backup：the application-owned complete-store authority owns the deterministic manifest, SHA-256 entries, M1/M2 metadata and intelligence/provenance coverage, complete pre-restore validation, new-root staging and no-silent-merge policy; this batch adds no second writer or dependency.
